...
class Board:
    ...
    def legalmoves(self):
        ...
        for move in np.argwhere(self.board == otherplayer):
            ...
            for direction in range(0, 8):
                ...
                if (playablespace[0] in range(0, 8)) and (
                    playablespace[1] in range(0, 8)
                ):
                    ...
                    try:
                        if self.board[tile[0], tile[1]] == 0:
                            ...
                            while backtrack:
                                try:
                                    if (
                                        self.board[tile[0], tile[1]] == self.player
                                    ) or (self.board[tile[0], tile[1]] == 0):
                                        backtrack = False
                                ...
                                except:  # you MUST say what exception you are triggering
                                    # on here - this is likely to
                                    # be a source of bugs though,
                                    # even if you do. Presumably
                                    # the error is array index
                                    # out of bounds
                                    backtrack = False

                            if not backtrack and (
    ...
    def win(self):
        zero = 0
        X = 0  # rename to something like x_count; o_count
        O = 0
    ...
    def move(self, position):
        ...
        if position in self.legalmoves():
            ...
            for direction in range(0, 8):
                ...
                while (
                    (tile[0] in coords)
                    and (tile[1] in coords)
                    and not END
                    and (self.board[tile[0], tile[1]] != 0)
                ):
...
class Nodes:
    def __init__(self, result):
        ...
        self.score = 0
        self.loss = 0
        if not b.win() or ((len(b.legalmoves())) == 0):
        ...
        self.children = [
            -math.inf for _ in range(64)
        ]  # set to 64 for the number of squares
...
def ucb1(current_node, parent_node, turn):
    ...
    if current_node.visits == 0:
        ...
        if turn == 1:
            UCB = float(current_node.score / current_node.visits) + 200 * math.sqrt(
                (np.log(parent_node.visits)) / current_node.visits
            )
        else:
            UCB = float(current_node.loss / current_node.visits) - 200 * math.sqrt(
                (np.log(parent_node.visits)) / current_node.visits
            )  # loss not score
    return UCB
...
def selection(current_node, List):
    ...
    if (current_node.terminal == True) or Leaf:
        return current_node
...
def rollout(current_node):
    ...
    while not b.win():  # removed --> and (len(b.legalmoves()) != 0)
        try:
            moves = b.legalmoves()
